...
- Verified: demo-rollout shows orders/payments ACTED (autonomous / assisted-approved), api-gateway/inventory escalated (assisted-held / gated), and the silent notifications failure resolved END TO END autonomously (queue 100 -> 0 after the agent restarts the worker). test_resume updated (2 action rows now: proposal + remediation, both idempotent). 3 rollout tests. Full suite 58/58. Suggested tag: ch12. PART IV nearly done (ch13 multi-agent remains).
